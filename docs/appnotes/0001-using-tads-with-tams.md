...
[TAMS](https://github.com/bbc/tams) is the Time-Addressable Media Store API specification.
...
Both TADS and TAMS use nanosecond resolution timestamps and timeranges to refer to timelines.
Related content in TADS and TAMS should use a shared timeline.
For the case where data in TADS annotates media in TAMS, the TADS Track uses the same timeline as the referenced media in TAMS.
...
As TAMS Sources/Flows each use a single unambiguous timeline, `timeline_reference.timeline_selected` shouldn't be populated.
...
`adb87970-b03a-4266-b5d8-f3f6541fb73a` in `schema_id` is the ID of the transcript Payload Schema available in this TADS Service Instance.
...
TADS Tracks may be filtered by their `timeline_reference.entity_id` using the `timeline_reference.entity_id` query parameter on the `GET /tracks` method, which accepts a comma-seperated list of percent-encoded URIs.
...
The use of TAMS for the `timeline_reference.entity_id` may be identified by the use of the `tams://` "pseudo-protocol" at the start of the URI.
...
Performing a GET request against it will return the Source/Flow metadata, with other TAMS endpoints such as `flows/<flow_id>/segments` being available on the same host.

## Potential Architectures

### Batch Analysis of TAMS Media

Where a process is to analyse a TAMS Flow whose `status` is `closed_complete`, this may be performed purely via the REST API.
The process shall retrieve and analyse every Segment in the TAMS Flow.
The timing information for resultant Track Events should be derived from the Segment metadata.
Resulting analysis data may then be written as a list of Track Events to TADS.
...
Where the Flow to be analysed is still being ingested, it may be more appropriate for the analysis process to register a webhook to recieve `flows/segments_added` webhook events from TAMS as new Flow Segments become available.
...
Again, this may be a batch process using the TADS REST API, or it may be event-driven using the TADS Webhooks capability.
...
The `flows/segments_added` webhook events for this Flow are used to trigger a face detection algorithm.
The results of this are written to a Track in TADS.
The `tracks/events_added` webhook events for this Track are then used alongside the original Flow to produce cropped video of the faces present.
That cropped video is then written back to TAMS.
...
TADS Tracks may explicitly indicate they do not contain overlapping Track Events by setting the `allow_overlaps` attribute to `false`.
...
TADS Tracks may explicitly indicate they do not permit editing of Track Events by setting the `editable_events` attribute to `false`.
...
Likewise, a webhook event-driven process may subscribe to `tracks/events_added` but not `tracks/events_updated` or `tracks/events_deleted`.
...
For the reasons stated above, workflows that use TADS data to generate media to be written to TAMS should prefer Tracks that are configured with both `allow_overlaps` and `editable_events` set to `False`.
...
1. A transcript worker subscribes to the `flows/created` Webhook Events in TAMS with appropriate filters (e.g. tags) to identify Flows to be transcribed
2. When a Flow created Webhook Event is received, a Track is created in TADS to write the transcript to
    1. The `schema_id` is set to the ID of an appropriate Schema
    2. The `timeline_reference.entity_id` is set to the URI of the Flow's Source
3. The worker subscribes to the `flows/segments_added` Webhook Events in TAMS with the `flow_ids` filter set to the ID of the Flow to be transcribed
...
5. The resulting transcript is then written as Track Events to the Track created earlier
    1. The TimeRange of each Track Event is time that transcript payload covers against the Flow's timeline
...
7. The Client should verify that the `timeline_reference.entity_id` URI is a TAMS URI
8. The Source ID in the Track's `timeline_reference.entity_id` is used to identify the most appopriate Flow for retrieval of the media
    1. This is done via `GET https://tams.example.com/flows?source_id=<source_id>` with the corresponding Source ID
9. The TimeRange of the Track Events of interest is then used to retrieve the corresponding Flow Segments
    1. This is done via `GET https://tams.example.com/flows/<flow_id>/segments?timerange=<timerange>` with the corresponding Flow ID and TimeRange
