...
update-api-version:
	docker run --user="root" --rm -v "$(topdir)/api":/workdir mikefarah/yq e ".info.version = \"$(NEXT_VERSION)\" | .servers[1].variables.version.default = \"v$(NEXT_VERSION)\"" -i TimeAddressableDataStore.yaml
